...
import { CheckCircle2, XCircle, Type, AlignLeft, ImagePlus, Tags, Send, Plus } from 'lucide-react'
      ...
      <div className="min-h-screen bg-black text-white p-8">
        ...
        <div className="max-w-xl mx-auto pt-20">
          ...
          <p className="text-red-500 text-sm">
            Cloudinary configuration is missing. Please set environment variables.
          </p>
        </div>
      </div>
    )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/posts/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, content, coverImage: imageUrl, categories: selectedCategories }),
      })

      if (!response.ok) throw new Error('Failed to create post')

      setTitle(''); setContent(''); setImageUrl(''); setSelectedCategories([])
      toast.success('Post created successfully!', { icon: <CheckCircle2 className="w-5 h-5 text-green-500" /> })
    } catch (error) {
      console.error('Error creating post:', error)
      toast.error('Failed to create post. Please try again.', { icon: <XCircle className="w-5 h-5 text-red-500" /> })
    } finally {
      ...
      <div className="p-4 sm:p-6 pt-24 sm:pt-28 pb-20 sm:pb-24">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex flex-col gap-1">
            <h1 className="text-2xl font-bold tracking-tight text-white/90">Create Post</h1>
            <p className="text-sm text-white/50">Draft a new article for your blog.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] space-y-3 group transition-colors hover:bg-white/[0.03]">
              ...
              <input
                ...
                className="w-full bg-transparent text-xl font-medium text-white placeholder:text-white/20 border-none focus:border-transparent outline-none focus:outline-none focus:ring-0 shadow-none px-0 py-1"
                required
              />
            </div>
